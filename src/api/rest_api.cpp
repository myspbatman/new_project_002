...
size_t RestApi::handle(const char* method, const char* path, char* out, size_t cap,
    ...
    if (!strcmp(method, "GET") && !strcmp(path, "/api/v1/battery")) {
        if (m.sensor_ok) n = snprintf(out, cap, "{\"voltage\":%.3f,\"current\":%.3f,\"power\":%.3f,\"capacity_ah\":%.3f,\"capacity_mah\":%.0f,\"remaining_ah\":%.3f,\"remaining_mah\":%.0f,\"soc_percent\":%.2f,\"soc_valid\":%s,\"consumed_ah\":%.6f,\"consumed_mah\":%.3f,\"consumed_wh\":%.6f,\"sensor_ok\":true,\"timestamp_ms\":%llu}", m.voltage,m.current,m.power,m.capacity_ah,m.capacity_ah*1000,m.remaining_ah,m.remaining_ah*1000,m.soc_percent,m.soc_valid?"true":"false",m.consumed_ah,m.consumed_ah*1000,m.consumed_wh,(unsigned long long)m.timestamp_ms);
        else n = snprintf(out, cap, "{\"voltage\":0,\"current\":0,\"power\":0,\"capacity_ah\":%.3f,\"capacity_mah\":%.0f,\"remaining_ah\":%.3f,\"remaining_mah\":%.0f,\"soc_percent\":%.2f,\"soc_valid\":%s,\"consumed_ah\":%.6f,\"consumed_mah\":%.3f,\"consumed_wh\":%.6f,\"sensor_ok\":false,\"error\":\"%s\",\"timestamp_ms\":%llu}",m.capacity_ah,m.capacity_ah*1000,m.remaining_ah,m.remaining_ah*1000,m.soc_percent,m.soc_valid?"true":"false",m.consumed_ah,m.consumed_ah*1000,m.consumed_wh,m.error?m.error:"sensor_error",(unsigned long long)m.timestamp_ms);
        ...
        n = snprintf(out,cap,"{\"device\":\"%s\",\"firmware\":\"%s\",\"ip\":\"%s\",\"sensor\":{\"type\":\"%s\",\"ok\":%s},\"battery\":{\"voltage\":%.3f,\"current\":%.3f,\"power\":%.3f,\"capacity_mah\":%.0f,\"remaining_mah\":%.0f,\"soc_percent\":%.2f,\"soc_valid\":%s},\"timestamp_ms\":%llu}",config::DEVICE_NAME,config::FIRMWARE_VERSION,ip,battery_.sensor().typeName(),m.sensor_ok?"true":"false",m.voltage,m.current,m.power,m.capacity_ah*1000,m.remaining_ah*1000,m.soc_percent,m.soc_valid?"true":"false",(unsigned long long)m.timestamp_ms);
